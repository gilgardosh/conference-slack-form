...
export class RateLimiter {
   ...
   */
  checkAndIncrement(key: string, limit: number, windowSec: number): RateLimitResult {
    const now = Date.now();
   ...
   */
  getStatus(key: string, limit: number): { count: number; remaining: number; resetAt: number | null } {
    const entry = this.store.get(key);
    const now = Date.now();
 ...
 */
export function checkIpRateLimit(ip: string, limit: number, windowSec: number): RateLimitResult {
 ...
 * @returns Rate limit result
 */
export function checkEmailRateLimit(email: string, limit: number, windowSec: number): RateLimitResult {
