...
describe('Rate Limiter Integration', () => {
  ...
  describe('IP-based rate limiting', () => {
    it('should allow requests within IP limit', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        expect(response.headers.get('X-RateLimit-Remaining-IP')).toBe((2 - i).toString());
      }
    });
    ...
    it('should handle different IPs independently', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        const request = createRequest({ ...validFormData, email: `user${i}@example.com` }, ip1);
        await worker.fetch(request, mockEnv);
      }
      
      // IP1 should be blocked
      const blockedRequest = createRequest({ ...validFormData, email: 'blocked@example.com' }, ip1);
      const blockedResponse = await worker.fetch(blockedRequest, mockEnv);
      expect(blockedResponse.status).toBe(429);
      
      // IP2 should still be allowed
      const allowedRequest = createRequest({ ...validFormData, email: 'allowed@example.com' }, ip2);
  ...
  describe('Email-based rate limiting', () => {
    it('should allow requests within email limit', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        // Check rate limit headers
        expect(response.headers.get('X-RateLimit-Remaining-Email')).toBe((2 - i).toString());
      }
    });
    ...
    it('should handle different emails independently', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        const request = createRequest({ ...validFormData, email: email1 }, `192.168.1.${i + 50}`);
        await worker.fetch(request, mockEnv);
      }
      
      // email1 should be blocked
      const blockedRequest = createRequest({ ...validFormData, email: email1 }, '192.168.1.53');
      const blockedResponse = await worker.fetch(blockedRequest, mockEnv);
      expect(blockedResponse.status).toBe(429);
      
      // email2 should still be allowed
      const allowedRequest = createRequest({ ...validFormData, email: email2 }, '192.168.1.54');
      const allowedResponse = await worker.fetch(allowedRequest, mockEnv);
      expect(allowedResponse.status).toBe(200);
    });
  });
  ...
  describe('Combined rate limiting scenarios', () => {
    it('should enforce both IP and email limits', async () => {
      ...
      // Exhaust IP limit
      for (let i = 0; i < 3; i++) {
        const request = createRequest({ ...validFormData, email: `user${i}@example.com` }, ip);
        const response = await worker.fetch(request, mockEnv);
        expect(response.status).toBe(200);
      }
      
      // IP should be blocked even with new email
      const ipBlockedRequest = createRequest({ ...validFormData, email: 'newemail@example.com' }, ip);
