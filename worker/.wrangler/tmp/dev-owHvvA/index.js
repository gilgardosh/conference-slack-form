...
function errorResponse(errorCode, message, status = 400, metadata) {
  const body = {
    ok: false,
    errorCode,
    message,
...
var FormSubmissionSchema = external_exports.object({
  companyName: external_exports.string().min(1, "Company name is required"),
  email: external_exports.string().email("Invalid email format")
});
function sanitizeCompanyName(raw) {
  return raw.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "").replace(/-+/g, "-").replace(/^-+|-+$/g, "").slice(0, 67);
}
__name(sanitizeCompanyName, "sanitizeCompanyName");
...
__name(validateAndSanitize, "validateAndSanitize");
...
var RateLimiter = class {
   ...
   */
  checkAndIncrement(key, limit, windowSec) {
    const now = Date.now();
...
var SlackClient = class {
  ...
  logChannelId;
  fetchFn;
  constructor(token, teamId, logChannelId, fetchFn = fetch) {
    this.token = token;
    this.teamId = teamId;
   ...
   */
  async slackRequest(endpoint, body) {
    try {
   ...
   */
  async inviteGuest(email, channelId) {
    const result = await this.slackRequest(
      "admin.users.invite",
      {
        email,
        channel_ids: [channelId],
        guest_expiration_ts: Math.floor(Date.now() / 1e3) + 30 * 24 * 60 * 60,
        // 30 days
        is_restricted: true,
  ...
  async logToChannel(message, level = "info") {
    ...
    return {
      ok: true,
      timestamp: result.ts || (/* @__PURE__ */ new Date()).toISOString()
    };
  }
};
function createSlackClient(token, teamId, logChannelId, fetchFn) {
  return new SlackClient(token, teamId, logChannelId, fetchFn);
}
__name(createSlackClient, "createSlackClient");

// src/lib/email.ts
var EmailClient = class {
  static {
    __name(this, "EmailClient");
  }
  apiKey;
  fetchFn;
...
<body>
   ...
   */
  generateWelcomeEmailText(params) {
...
function createEmailClient(apiKey, fetchFn) {
  return new EmailClient(apiKey, fetchFn);
}
__name(createEmailClient, "createEmailClient");
async function sendWelcomeEmail(params, apiKey, fetchFn) {
  if (!apiKey) {
    return {
      ok: false,
...
router.post("/api/submit", async (request, env) => {
  try {
    ...
    const rateLimitWindowSec = parseInt(env.RATE_LIMIT_WINDOW_SEC || "3600", 10);
    ...
    if (!ipRateLimit.allowed) {
      return new Response(JSON.stringify({
        ...
        headers: {
          ...
          "X-RateLimit-Reset": Math.ceil(ipRateLimit.resetAt / 1e3).toString()
        }
      });
    }
    ...
    if (!emailRateLimit.allowed) {
      return new Response(JSON.stringify({
        ...
        headers: {
          ...
          "X-RateLimit-Reset": Math.ceil(emailRateLimit.resetAt / 1e3).toString()
        }
      });
    }
    ...
    const logMessage = partialFailures.length > 0 ? `Submission processed for ${email} (company: ${validationResult.value.companyName} -> ${sanitizedCompanyName}), channel: ${channelName} (${channelId}). Partial failures: ${partialFailures.join(", ")}` : `Submission successfully processed for ${email} (company: ${validationResult.value.companyName} -> ${sanitizedCompanyName}), channel: ${channelName} (${channelId})`;
    ...
    return jsonResponse(response, 200, {
      ...
      "X-RateLimit-Reset": Math.ceil(Math.max(ipRateLimit.resetAt, emailRateLimit.resetAt) / 1e3).toString()
    });
  } catch (error) {
